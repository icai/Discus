...
String.prototype.breakWord = function(f, c) {
	c || (c = "<wbr/>");
	return this.replace(RegExp("(\\w{" + (f ? f : 0) + "})(\\w)", "g"), function(i, g, b) {
		return g + c + b
	})
};
...
qq = window.qq || {
	...
	evalScript: function(a) {
		...
		(a = a.match(new RegExp(b, "img"))) && qq.each(a, function(c) {
	...
	removeClass: function(g, i) {
		if (g) {
			var h = i.split(" ");
			1 < h.length ? qq.each(h, function(b) {
				qq.removeClass(g, b)
	...
	html: function(g) {
		var f = qq.DC("div"),
			h = [];
		f.innerHTML = g;
		qq.each(f.childNodes, function(b) {
			h.push(b)
		});
		return h
	},
	...
	children: function(g, f) {
		...
		qq.each(g.childNodes, function(k) {
			var c = false;
			if (f) {
			...
			if (i) {
				if (void 0 == f) {
					if (window.getComputedStyle) {
						return h = h.replace(/([A-Z])/g, "-$1"), h = h.toLowerCase(), window.getComputedStyle(i, null).getPropertyValue(h)
					}
					if (i.currentStyle) {
						return "opacity" == h ? 0 <= i.style.filter.indexOf("opacity=") ? parseFloat(i.style.filter.match(/opacity=([^)]*)/)[1]) / 100 : "1" : i.currentStyle[h]
					}
				} else {
					!b && "opacity" == h && (b = "opacity" in i.style ? 1 : 2), "opacity" == h && 2 == b ? i.style.filter = (i.filter || "").replace(/alpha\([^)]*\)/, "") + "alpha(opacity=" + 100 * f + ")" : i.style[h] = f
				}
			}
		}
	}(),
				...
				if (document.addEventListener) {
					...
					if (document.attachEvent) {
						...
						if (b) {
							...
							document.documentElement.doScroll && !b && function() {
								if (!qq.ready.done) {
									try {
...
qq.B.ie && document.execCommand("BackgroundImageCache", false, true);
/*  |xGv00|d5aca1ae81e55002f0b4a495b476e2d2 */